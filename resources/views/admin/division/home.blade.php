    ...
    <body id="kt_body" style="background-image: url(adminassets/dist/assets/media/patterns/header-bg.png)" class="header-fixed header-tablet-and-mobile-fixed toolbar-enabled">
				...
				<div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
					...
					<div id="kt_header" class="header align-items-stretch" data-kt-sticky="true" data-kt-sticky-name="header" data-kt-sticky-offset="{default: '200px', lg: '300px'}">
						...
						<div class="container-xxl d-flex align-items-center">
							...
							<div class="header-logo me-5 me-md-10 flex-grow-1 flex-lg-grow-0">
								<a href="#">
									<img alt="Logo" src="assets/logochu-new.png" class="h-15px h-lg-50px logo-default" />
									<img alt="Logo" src="assets/logochu-new.png" class="h-15px h-lg-50px logo-sticky" />
								</a>
							</div>
							<!--end::Header Logo-->
							<!--begin::Wrapper-->
					...
					<div class="toolbar py-5 py-lg-15" id="kt_toolbar">
						...
						<div id="kt_toolbar_container" class="container-xxl d-flex flex-stack flex-wrap">
							<!--begin::Title-->
							<h3 class="text-white fw-bolder fs-2qx me-5">Gestion des divisions</h3>
							<!--begin::Title-->
							<!--begin::Actions-->
							<div class="d-flex align-items-center flex-wrap py-2">
                                ...
                                <div >
									...
									<span class="buttoncss" >Ajouter division</span>
									</a>
								</span>	
								</div>
					...
					@endif
					<form action="{{ url('filter-division') }}" method="GET">
						<div class="form-group">
							<label for="hopital">Trier par Hôpital :</label>
							<select name="hopitalId" id="hopital" class="form-control" >
								<option value="" >Tous</option>
								@foreach($hopital as $hopital)
									<option value="{{ $hopital->id }}" @if($hopital->id == $hopitalId) selected @endif>{{ $hopital->nom_hopital }}</option>
								@endforeach
							</select>
						</div>
    				...
    				<tbody>
						...
						@foreach($division as $data)
        					...
        					<tr class="active-row">
								...
								<td>{{$data->nom_division}}</td>
								<td>{{$hopital->where('id', $data->hopital_id)->pluck('nom_hopital')->first() }}</td>
								<td><center>
								<a href="{{url('delete_division',$data->id)}}" onclick="event.preventDefault(); showConfirmationModal({{$data->id}});" class="btn">Supprimer</a>
								<a href="{{url('update_division',$data->id)}}" class="btn">Modifier</a>
							</center></td>
        					</tr>
						@endforeach
						@else
						<tr>
							<td class="text-center" colspan="5">Aucune division Trouvée</td>
						</tr>
					@endif
				
    				</tbody>
					</table>
					{{ $division->appends(request()->query())->links() }}
					</div>
		...
		<div id="confirmationModal" class="modal">
            ...
            <div class="modal-content">
                <h3>Confirmation</h3>
                <p>Etes-vous sûr(e) de vouloir supprimer cette division?</p>
